...
class RTopo(Topo):
    ...
    def build(self, **_opts):     # special names?
        ...
        if len(argv) > 1:
            topo_file = argv[1]
        f = open(topo_file, 'r')
        n, m, k, queue = [int(x) for x in f.readline()[:-1].split()]
        dic = {}
        for i in range(n):
            name = f.readline()[:-1]
...
def main():
    ...
    # h1.cmd('python3 sender.py 10000 10.0.3.10 5430 reno&')
    # h2.cmd('netcat -l 2345&')
    # h2.cmd('python3 sender.py 10000 10.0.3.10 5431 vegas&')
